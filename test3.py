		...
		for socks in read_sockets:
			if socks == server:
				...
				if(firstmessage):
					...
					temp = message.split(";")
					#the_loc.lat = float(temp[0])
					#the_loc.lon = float(temp[1])
					lonTest = float(temp[0])/100
					latTest = float(temp[1])/100
			else:
				#message = sys.stdin.readline()
				message =  vehicle.location.global_frame.lat + ";" + vehicle.location.global_frame.lon + ";" + str(time.time()-base_time) 
...
lonTest = 0
latTest = 0
home = LocationGlobalRelative(vehicle.location.global_frame.lat,vehicle.location.global_frame.lon,alt)
server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
ip_addr = '192.168.1.145'
port = 50000
time.sleep(15)
...
preChecks()
armVehicle()
print("AFTER armVehicle")
#set groundspeed
vehicle.groundspeed = 5


the_loc = get_location_metres(home,40,40)

fieldnames = ['Latitude,Longitude,Timestamp']
with open('/boot/TestData.csv', mode='w') as csv_file:
...
while True:
	#print("IN WHILE LOOP")
	vehicle.simple_goto(the_loc)
	#if((time.time()-c_time) > 5):
	#	the_loc=home
	#send_ned_velocity((vehicle.location.global_frame.lon+lonTest),(vehicle.location.global_frame.lat+latTest),0,(1))
	with open('/boot/TestData.csv', mode='a') as csv_file:
		writer = csv.writer(csv_file, delimiter=',')
		writer.writerow([vehicle.location.global_frame.lat,vehicle.location.global_frame.lon,time.time()-base_time])
	#send_ned_velocity(2,0,0,3)
#	print(str(the_loc.lat) + ";" + str(the_loc.lon))
	time.sleep(2)


#def GetLocationOffsetMeter(origin, dNorth, dEast, alt):
...
"""#SimpleGoToNorth
print ("Setting up and going to...")
sampleLat = vehicle.location.global_frame.lat+0.0002

while(vehicle.location.global_frame.lat < sampleLat):
	vehicle.simple_goto(theLoc)
	time.sleep(1)

print ("Setting up and going to...")
sampleLat2 = vehicle.location.global_frame.lat-0.0002
theLoc2 = LocationGlobal(sampleLat2,vehicle.location.global_frame.lon,32.839355)

while(vehicle.location.global_frame.lat > sampleLat2):
	vehicle.simple_goto(theLoc2)
	time.sleep(1)"""


#vehicle.mode = VehicleMode("HALT")
